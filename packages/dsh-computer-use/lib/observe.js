 ...
 */
async function commitScreenshot(ctx, target, state, cfg, sid) {
...
async function describeVision(ctx, cfg, state, target, exec, sid, question) {
  ...
  if (llm && attachments) {
    try {
      const { ref, compact } = await commitScreenshot(ctx, target, state, cfg, sid)
 ...
 */
export async function screenObserve(ctx, args, cfg, exec, sid) {
  const wantMode = args.mode
  const maxElements = args.maxElements || cfg.maxElements || 500

  // 1. 窗口列表
  const winList = await cuaCall('list_windows', { on_screen_only: true }, sid)
  ...
  let state = await cuaCall('get_window_state', gwArgs, sid)
  ...
  if (wantMode === 'native') {
    ...
    try {
      const { ref, width, height, compact } = await commitScreenshot(ctx, target, state, cfg, sid)
      image = {
    ...
    if (degraded) {
      if (await routeImageCapable(ctx, exec)) {
        try {
          const { ref, width, height } = await commitScreenshot(ctx, target, state, cfg, sid)
 ...
 */
export async function screenZoom(ctx, args, cfg, exec, sid) {
  const capable = await routeImageCapable(ctx, exec)
  if (!capable) {
